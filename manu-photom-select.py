...
def main(full: "Compute full dataset"=False):
    ...
    THRESH = 1.e-13                 # Threshold for possible
                                    # saturation of long exposures
    ...
    for longband, band in ("red", "r"), ("green", "g"), ("blue", "b"):
        thdu = {
            "long":
            fits.open("Manu-Data/M42_{}/table_M42_l{}.fits".format(longband, band))[1],
            "short":
            fits.open("Manu-Data/M42_{}/table_M42_s{}.fits".format(longband, band))[1],
        }
        hdu = {
            "long":
            fits.open("Manu-Data/M42_{}/M42_l{}.fits".format(longband, band))[0],
            "short":
            fits.open("Manu-Data/M42_{}/M42_s{}.fits".format(longband, band))[0],
        }
        ...
        for spectrum, spectrum_s, metadata in zip(specdata, specdata_s, tabdata):
            ...
            section["band"] = band
            # Multiply by 1e15 as with the Helix to make the spectra
            # of order unity for weak lines
            section["mean"] = 1.e15*np.where(
                brightmask, spectrum_s, spectrum)*metadata["factor2"]
            ...
            # We don't have a good estimate of the std of the data -
            # so make something up!
            section["std"] = 0.01*np.ones_like(spectrum)
            # Fit continuum to the clean wav ranges
            section["cont"] = fit_continuum(
                wavs[band], section["mean"], cmask[band], npoly=2)
