...
|------+------+--------+--------+--------+------+------+-------+---------+----------|
| 5755 | 6584 | FQ575N | F658N  | F547M  | 0.14 |  3.0 |   2.5 |    0.94 |     1.01 |
...
|--------+--------+--------+---------+---------------|
| FQ575N | F658N  | F547M  |   0.036 |         0.057 |
...
#+BEGIN_SRC python :return plotlinks :results list
  ...
  for wav1, wav2, FI, FII, FIII, _, _, _, kI, kII in intable[0:]:
      ...
      m = ssmask & np.isfinite(x) & np.isfinite(y) & ( xmin < x ) & (x < xmax) & (ymin < y) & (y < ymax)
      x, y = x[m], y[m]
      pltfile = "contam-correct-{}-{}-{}.pdf".format(FI, FII, FIII)
      plt.hist2d(x.ravel(), y.ravel(), bins=200, range=[[xmin, xmax], [ymin, ymax]], cmap=plt.cm.gray_r)
      plt.plot([xmin, ymin], [xmax, ymax], '--k', zorder=100)
