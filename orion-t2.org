...
#+BEGIN_SRC python :return plotlinks :results output
  ...
  for n in nlist:
      for wav1, wav2, FI, FII, FIII, _, _, _, kI, kII, kIII in intable[-2:]:
          filterset = {"wav1": wav1, "wav2": wav2, "I": FI, "II": FII, "III": FIII}
...
#+BEGIN_SRC python :return plotfile :results file
  ...
  for n, ax in zip([1, 4, 16, 64], axes.flat):
      ...
      radii = fits.open(
          "radii_bin{0:03d}x{0:03d}.fits".format(n))[0]
      m = include.get_mask(hdu=r658) & (~exclude.get_mask(hdu=r658))
      m = convex_hull_image(m)
      x = radii.data[m]
      y = r658.data[m]
      w = s656.data[m]
...
#+BEGIN_SRC python :return plotfile :results file
  ...
  for n, ax in zip([1, 4, 16, 64], axes.flat):
      ...
      radii = fits.open(
          "radii_bin{0:03d}x{0:03d}.fits".format(n))[0]
      m = include.get_mask(hdu=r502) & (~exclude.get_mask(hdu=r502))
      m = convex_hull_image(m)
      x = radii.data[m]
      y = r502.data[m]
      w = s656.data[m]
