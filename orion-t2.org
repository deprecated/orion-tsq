...
**** TODO [3/4] Getting the normalizations right
...
+ So we should multiply all our measured rates by 1.5
***** TODO Use pysynphot to double-check my normalizations
...
**** TODO [4/6] Little tweaks to improve things
...
|        |  ODH | Manu | Adal | Ring | ODH' | Manu' | Adal' | Ring' | Average'      |   |
|--------+------+------+------+------+------+-------+-------+-------+---------------+---|
| F658N  | 0.73 | 0.89 | 0.83 | 0.55 | 0.90 |  1.02 |  0.90 |  0.98 | 0.95 +/- 0.03 | ? |
| F656N  | 0.92 | 0.95 | 0.93 | 0.58 | 1.14 |  1.09 |  1.01 |  1.04 | 1.07 +/- 0.03 | * |
| F673N  | 0.80 | 0.73 | 0.81 | 0.58 | 0.99 |  0.84 |  0.88 |  1.04 | 0.94 +/- 0.05 |   |
| F502N  | 0.83 | 0.91 | 0.91 | 0.53 | 1.02 |  1.05 |  0.99 |  0.95 | 1.00 +/- 0.02 | * |
| F487N  | 0.81 | 0.88 | 0.86 | 0.57 | 1.00 |  1.01 |  0.93 |  1.02 | 0.99 +/- 0.02 |   |
| F469N  | 0.73 | 0.61 | 0.88 | 0.76 | 0.90 |  0.70 |  0.96 |  1.36 | 0.98 +/- 0.14 | ? |
| F547M  | 0.81 | 0.87 | 0.92 | 0.56 | 1.00 |  1.00 |  1.00 |  1.00 | 1.00          |   |
| FQ575N | 0.91 | 0.79 | 0.92 | 0.61 | 1.12 |  0.91 |  1.00 |  1.09 | 1.03 +/- 0.05 |   |
| FQ672N | 0.79 | 0.75 | 0.84 | 0.48 | 0.98 |  0.86 |  0.91 |  0.86 | 0.90 +/- 0.03 | * |
| FQ674N | 0.88 | 0.79 | 0.87 | 0.46 | 1.09 |  0.91 |  0.95 |  0.82 | 0.94 +/- 0.06 |   |
| FQ437N | 0.76 | 0.61 | 0.84 | 0.66 | 0.94 |  0.70 |  0.91 |  1.18 | 0.93 +/- 0.10 |   |
| FQ436N | 0.78 | 0.66 | 0.87 | 0.55 | 0.96 |  0.76 |  0.95 |  0.98 | 0.91 +/- 0.05 |   |
|--------+------+------+------+------+------+-------+-------+-------+---------------+---|
| F645N  |  nan |  nan |  nan | 0.52 |  nan |   nan |   nan |  0.93 | nan +/- nan   |   |
|--------+------+------+------+------+------+-------+-------+-------+---------------+---|
| mean   |      |      |      |      | 1.00 |  0.90 |  0.95 |  1.03 | 1.00          |   |
| std    |      |      |      |      | 0.08 |  0.13 |  0.04 |  0.14 | 0.10 +/- 0.02 |   |
...
+ Manu's F547M needs sorting out - it runs off the end of the wav scale, so we are missing some flux
  + this will reduce the value from 0.87 by a bit, so that the primed values will all be highre
...
#+BEGIN_SRC python :noweb yes :results file 
  ...
  plt.plot([0.0, x[m].max()], [0.0, gradient*x[m].max()], '-',
           label="gradient: {:.2f}".format(gradient))
  plt.plot([0.0, x[m].max()], [0.0, x[m].max()], 'r-',
           label="gradient: 1.0")
...
#+header: :results file
#+header: :var PA=60 :var floor=0.0 fudge=0.51 shift=0.0 stretch=1.0
#+BEGIN_SRC python :var fn="F658N" 
  import numpy as np
  from pathlib import Path
  from astropy.table import Table
  ...
  plt.plot(fulltab['x0_w'][m]*stretch + shift, fulltab[fn+'_w'][m], 'r', label='WFC3')
  plt.ylim(0.0, None)
  # plt.ylim(0.0, 5.0*np.median(fulltab[fn+'_s'][m] + floor))
  plt.grid()
...
******* F658N Ring profiles
#+call: ring-profile-test(fn="F658N", PA=60, fudge=0.56, stretch=1.0) :results file

#+RESULTS:
[[file:ring-profile-pa60-F658N.pdf]]

#+call: ring-profile-test(fn="F658N", PA=150, fudge=0.56, shift=-0.7, stretch=1.02) :results file
...
******* F547M Ring profiles
#+call: ring-profile-test(fn="F547M", PA=60, fudge=0.56, stretch=1.02) :results file

#+RESULTS:
[[file:ring-profile-pa60-F547M.pdf]]

#+call: ring-profile-test(fn="F547M", PA=150, fudge=0.56, shift=-0.7, stretch=1.02) :results file
