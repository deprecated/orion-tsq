...
#+RESULTS:
: Saving full_F502Nbin001x001.fits
: Saving full_F502Nbin002x002.fits
: Saving full_F502Nbin004x004.fits
: Saving full_F502Nbin008x008.fits
: Saving full_F502Nbin016x016.fits
: Saving full_F502Nbin032x032.fits
: Saving full_F502Nbin064x064.fits
: Saving full_F502Nbin128x128.fits
: Saving full_F502Nbin256x256.fits
...
#+BEGIN_SRC python :return plotfile :results file
  ...
  for n, ax in zip([1, 4, 16, 64], axes.flat):
      ...
      s673 = fits.open(
          "full_F673Nbin{0:03d}x{0:03d}.fits".format(n))[0]
      radii = fits.open(
          "radii_bin{0:03d}x{0:03d}.fits".format(n))[0]
...
#+BEGIN_SRC python :return outtab
  from astropy.io import fits
  import numpy as np
  x = fits.open("full_F656Nbin{0:03d}x{0:03d}.fits".format(1))[0].data
  m = np.isfinite(x) & (x > 0.0)
  x = np.log10(x[m])
  outtab = [["mean", "median", "std", "min", "max"]]
  outtab.append(None)
...
|   mean | median |    std |    min |        max |
|--------+--------+--------+--------+------------|
| 7.9913 | 8.1588 | 1.9101 | 0.0000 | 86526.4867 |
...
#+BEGIN_SRC python :return plotfile :results file
  ...
  for n, ax in zip([1, 4, 16, 64], axes.flat):
      ...
      m = convex_hull_image(m)
      x = radii.data[m]
      y = s656.data[m] * 10**(chb.data[m]*(1.0 - 0.220)) * 0.08*(x/100)**1.4
      w = s656.data[m]
      mm = np.isfinite(x) & (x > x1) & (x < x2) & (y > y1) & (y < y2)
      H, xedges, yedges = np.histogram2d(x[mm], y[mm],
                                         [20, 20], [[x1, x2], [y1, y2]],
...
#+BEGIN_SRC python :return plotfile :results file
  ...
  #    plt.plot(xgrid, sig_width, c, label=None)
      plt.plot(xgrid, sig_width, c, lw=3, label="binning {0} x {0}".format(n))
