...
*** TODO [0/6] Absolute calibration of the filters
...
**** TODO [0/0] Getting the normalizations right
...
+ Fix the positions of the slits - I don't believe that they are all centered on RA of th1C
...
+ [ ] Now we need to go back and re-run the odh programs, taking into account the shifts
...
#+BEGIN_SRC python :noweb yes :results file 
  ...
  if 'Q' in fn:
      mm = mm & find_sweetspot_mask(xx, yy) 
  odh_flux = odhtab[fn + '_s'][mm]

  axes[0].scatter(xx[mm], yy[mm], c=odh_flux, vmin=0.0, vmax=fmax, s=50, alpha=0.6, marker='s')
  scat0 = axes[0].scatter(x[m], y[m], c=flux, vmin=0.0, vmax=fmax, alpha=0.6)
  axes[0].set_title('Manu/ODH Spectra: ' + fn)
  fig.colorbar(scat0, ax=axes[0])
  flux = fulltab[fn + '_w'][m]
  fmax = maxmeds*np.median(flux)
  odh_flux = odhtab[fn + '_w'][mm]
  scat1 = axes[1].scatter(xx[mm], yy[mm], c=odh_flux, vmin=0.0, vmax=fmax, s=50, alpha=0.6, marker='s')
  ...
  # inset plot with the S30, S60 profiles
  for yslit, vport, xshift, yshift  in [[30.0, [0.2, 0.1, 0.2, 0.2], 10.0, -3.0],
                                        [60.0, [0.7, 0.1, 0.2, 0.2], 8.0, 0.0],
                                        [90.0, [0.4, 0.7, 0.2, 0.2], 7.0, 0.0]]:
      ax_inset = plt.axes(vport, axisbg='yellow')
      mms = mm & (yy == -yslit)
      mf = m & (np.abs(y + yslit + yshift) <= 3.0) 
      ax_inset.plot(xx[mms]+xshift,  odhtab[fn + '_s'][mms], '.-',
                    label='spectra, xshift = {:.0f}'.format(xshift))
      ax_inset.plot(xx[mms],  4*odhtab[fn + '_w'][mms], '.-', label='WFC3')
      ax_inset.plot(x[mf],  fulltab[fn + '_s'][mf], '.', alpha=0.2,
                    label='fibers, yshift = {:.0f}'.format(yshift))
      ax_inset.plot(x[mf],  4*fulltab[fn + '_w'][mf], '.', alpha=0.2,
                    label='WFC3, yshift = {:.0f}'.format(yshift))
      ax_inset.tick_params(axis='both', which='major', labelsize='xx-small')
      ax_inset.set_xlim(xx.max(), xx.min())
      ax_inset.set_ylim(0.0, None)
...
#+call: manu-map-comparison(fn="F673N", band="red") :results file
...
#+call: manu-map-comparison(fn="F469N", band="blue") :results file

#+RESULTS:
[[file:manu-F469N-blue-maps.pdf]]

#+call: manu-map-comparison(fn="F469N", band="green") :results file

#+RESULTS:
[[file:manu-F469N-green-maps.pdf]]

#+call: manu-map-comparison(fn="FQ437N", band="blue") :results file

#+RESULTS:
[[file:manu-FQ437N-blue-maps.pdf]]

#+call: manu-map-comparison(fn="FQ436N", band="blue") :results file

#+RESULTS:
[[file:manu-FQ436N-blue-maps.pdf]]

#+call: manu-map-comparison(fn="FQ672N", band="red") :results file

#+RESULTS:
[[file:manu-FQ672N-red-maps.pdf]]

#+call: manu-map-comparison(fn="FQ674N", band="red") :results file
