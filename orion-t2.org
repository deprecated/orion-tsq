...
*** TODO [4/7] Absolute calibration of the filters
...
**** TODO [0/1] Getting the normalizations right
...
+ [ ] The problem is that I am a factor of 4 adrift
...
**** TODO [0/4] Little tweaks to improve things
+ [ ] Need to mask out some Ring positions
  + Particularly for FQ672N and FQ674N
+ [ ] Plate scale for Ring Nebula - need to expand it a bit
+ [ ] Zero point of Adal spectra
  + and others?
+ [ ] Split Manu dataset into different exposures
...
#+header: :var fn="FQ575N" :var band="red" :var maxmeds=3.0
#+BEGIN_SRC python :noweb yes :results file 
  ...
  aband = band if band in ['blue', 'red'] else 'blue'
  mmm = mmm & np.array([aband in s for s in adaltab['Section']])
  adal_flux = adaltab[fn + '_s'][mmm]

  fig = plt.figure(figsize=(16,8))
  ax_spec = plt.axes([0.06, 0.35, 0.25, 0.6])
  ax_wfc3 = plt.axes([0.36, 0.35, 0.25, 0.6], sharex=ax_spec, sharey=ax_spec)
  # ax_cb = plt.axes([0.56, 0.35, 0.02, 0.65])

  ax_spec.scatter(xx[mm], yy[mm], c=odh_flux, vmin=0.0, vmax=fmax, s=50, alpha=0.6, marker='s', edgecolors='none')
  ax_spec.scatter(xxx[mmm], yyy[mmm], c=adal_flux, vmin=0.0, vmax=fmax, s=50, alpha=0.6, marker='>', edgecolors='none')
  scat0 = ax_spec.scatter(x[m], y[m], c=flux, vmin=0.0, vmax=fmax, s=50, alpha=0.6, edgecolors='none')
  ax_spec.set_title('Manu/ODH Spectra: ' + fn)
  # fig.colorbar(scat0, ax=ax_spec)
  flux = fulltab[fn + '_w'][m]
  fmax = maxmeds*np.median(flux)
  odh_flux = odhtab[fn + '_w'][mm]
  adal_flux = adaltab[fn + '_w'][mmm]
  scat1 = ax_wfc3.scatter(xx[mm], yy[mm], c=odh_flux, vmin=0.0, vmax=fmax, s=50, alpha=0.6, marker='s', edgecolors='none')
  ax_wfc3.scatter(xxx[mmm], yyy[mmm], c=adal_flux, vmin=0.0, vmax=fmax, s=50, alpha=0.6, marker='>', edgecolors='none')
  ax_wfc3.scatter(x[m], y[m], c=flux, vmin=0.0, vmax=fmax, s=50, alpha=0.6, edgecolors='none')
  ax_wfc3.set_title('WFC3 Images: ' + fn)
  ...
  cb.set_label('WFC3 count rate, ADU/s/pixel')
  ...
  for yslit, vport in [[30.0, [0.68, 0.7, 0.25, 0.25]],
      ...
      ax_inset.plot(xx[mms],  4*odhtab[fn + '_w'][mms], '.-', label='WFC3')
      ...
      ax_inset.grid()
      ax_inset.set_title('S{:.0f} Profiles'.format(yslit), fontsize='x-small')
      ax_inset.legend(fontsize=4)
  ...
  for islit, vport in [[5, [0.06, 0.05, 0.25, 0.25]],
      ...
      ax_inset.plot(xxx[mmms],  4*adaltab[fn + '_w'][mmms], '.-', label='WFC3')
      # ax_inset.plot(x[mf],  fulltab[fn + '_s'][mf], '.', alpha=0.2, label='fibers')
      # ax_inset.plot(x[mf],  4*fulltab[fn + '_w'][mf], '.', alpha=0.2, label='WFC3 @ fibers')
      ax_inset.tick_params(axis='both', which='major', labelsize='xx-small')
...
#+call: manu-map-comparison(fn="F502N", band="green", maxmeds=2.0) :results file
...
#+call: manu-map-comparison(fn="F547M", band="green", maxmeds=2.0) :results file
...
#+name: adal-absolute
#+header: :var fn="F656N" :var band="red"  :var maxcount=1000.0
#+BEGIN_SRC python :noweb yes :results file 
  import numpy as np
  from astropy.table import Table
  import astropy.table
  ...
  plt.scatter(x[m], y[m], c=slitx[m], s=100.0/(islit[m]-4), alpha=0.6)
  plt.colorbar()
  plt.plot([0.0, x[m].max()], [0.0, gradient*x[m].max()], '-',
           label="gradient: {:.2f}".format(gradient))
  plt.xlabel("Adal spectra predicted count rate per pixel / s$^{-1}$")
  plt.ylabel("WFC3 image count rate per pixel / s$^{-1}$")
  plt.title(fn)
  plt.xlim(0.0, None)
  plt.ylim(0.0, None)
...
#+call: adal-absolute("F673N", "red", maxcount=10) :results file 

#+RESULTS:
[[file:adal-F673N-absolute.pdf]]

#+call: adal-absolute("F469N", "blue", maxcount=1.0) :results file 
...
#+call: adal-absolute("F547M", "red", maxcount=20) :results file 
...
#+name: ring-absolute
#+BEGIN_SRC python :noweb yes :results file :var fn="F656N" brightmax=1000.0
    import numpy as np
    ...
    gradient = y[m].sum()/x[m].sum()
    plt.scatter(x[m], y[m], c=d[m], s=fulltab['PA'][m], alpha=0.6)
    plt.colorbar()
    plt.plot([0.0, x[m].max()], [0.0, gradient*x[m].max()], '-',
             label="gradient: {:.2f}".format(gradient))
    plt.xlabel("Ring spectra predicted count rate per pixel / s$^{-1}$")
    plt.ylabel("WFC3 image count rate per pixel / s$^{-1}$")
    plt.title(fn)
    plt.xlim(0.0, None)
    plt.ylim(0.0, None)
...
#+call: ring-absolute("F547M", 1.0) :results file

#+RESULTS:
[[file:ring-F547M-absolute.pdf]]

#+call: ring-absolute("F658N") :results file

#+RESULTS:
[[file:ring-F658N-absolute.pdf]]

#+call: ring-absolute("F673N") :results file

#+RESULTS:
[[file:ring-F673N-absolute.pdf]]

#+call: ring-absolute("F502N") :results file

#+RESULTS:
[[file:ring-F502N-absolute.pdf]]

#+call: ring-absolute("F469N") :results file

#+RESULTS:
[[file:ring-F469N-absolute.pdf]]

#+call: ring-absolute("F487N") :results file

#+RESULTS:
[[file:ring-F487N-absolute.pdf]]

#+call: ring-absolute("F645N") :results file

#+RESULTS:
[[file:ring-F645N-absolute.pdf]]

#+call: ring-absolute("FQ575N") :results file

#+RESULTS:
[[file:ring-FQ575N-absolute.pdf]]

#+call: ring-absolute("FQ672N") :results file

#+RESULTS:
[[file:ring-FQ672N-absolute.pdf]]

#+call: ring-absolute("FQ674N") :results file

#+RESULTS:
[[file:ring-FQ674N-absolute.pdf]]

#+call: ring-absolute("FQ437N") :results file

#+RESULTS:
[[file:ring-FQ437N-absolute.pdf]]

#+call: ring-absolute("FQ436N") :results file
...
#+header: :results file
#+header: :var PA=60 :var floor=0.0 :var fudge=6
#+BEGIN_SRC python :var fn="F658N" 
  import numpy as np
  from pathlib import Path
  from astropy.table import Table
  ...
  # m = m & find_sweetspot_mask(fn, fulltab['dRA'], fulltab['dDEC']) 
  # m = m & (fulltab[fn+'_w'] < fulltab[fn+'_s']) 
  plt.plot(fulltab['x0_w'][m], fulltab[fn+'_s'][m] + floor, 'o', label='spectrum')
  plt.plot(fulltab['x0_w'][m], fudge*fulltab[fn+'_w'][m], 'r', label='WFC3')
  plt.ylim(0.0, None)
  # plt.ylim(0.0, 5.0*np.median(fulltab[fn+'_s'][m] + floor))
  plt.grid()
  plt.legend(loc='upper center', title='Ring Nebula: PA{}'.format(PA))
...
******* F547M Ring profiles
#+call: ring-profile-test(fn="F547M", PA=60) :results file

#+RESULTS:
[[file:ring-profile-pa60-F547M.pdf]]

#+call: ring-profile-test(fn="F547M", PA=150) :results file

#+RESULTS:
[[file:ring-profile-pa150-F547M.pdf]]

******* F645N Ring profiles
#+call: ring-profile-test(fn="F645N", PA=60) :results file

#+RESULTS:
[[file:ring-profile-pa60-F645N.pdf]]

#+call: ring-profile-test(fn="F645N", PA=150) :results file
...
#+name: odh-absolute
#+BEGIN_SRC python :noweb yes :results file :var fn="F547M"
  import numpy as np
  from astropy.table import Table
  import astropy.table
  from matplotlib import pyplot as plt
  ...
  gradient = y[m].sum()/x[m].sum()
  plt.scatter(x[m], y[m], c=ra[m], s=3000.0/np.abs(dec[m]), vmin=-120.0, vmax=0.0, alpha=0.6)
  plt.colorbar()
  plt.plot([0.0, x[m].max()], [0.0, gradient*x[m].max()], '-',
           label="gradient: {:.2f}".format(gradient))
  plt.xlabel("ODH spectra predicted count rate per pixel / s$^{-1}$")
  plt.ylabel("WFC3 image count rate per pixel / s$^{-1}$")
  plt.title(fn)
  plt.xlim(0.0, None)
  plt.ylim(0.0, None)
